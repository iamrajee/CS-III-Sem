...
void BFS(int a[][100],int source,int size,verp v){
    ...
    while(queueIsEmpty(&que)!=1){
        ...
        fo(j,0,size){
            if(a[u][j]!=0){
                //if unvisited
                if(v[j].dist==100000 || v[j].dist==v[u].dist+1){
                    
                  
                    v[j].dist=v[u].dist+1;
                    if(v[u].no_red+ (a[u][j]==1? 1 : 0 ) >= v[j].no_red){
                         v[j].no_red=v[u].no_red+ (a[u][j]==1? 1 : 0 );
		         v[j].par=u;
                  }
                    //v[j].color='G';
                    enQueue(&que, j);
...
void am_to_dot_red(int a[][100],int size,verp v,int source,int dest,FILE *foo){
    if(source==dest){
        ...
        if(a[dest][v[dest].par]==1)
	        fprintf(foo,"%d--%d [style=dotted,color=red];\n",dest,v[dest].par);
	if(a[dest][v[dest].par]==2)
        	fprintf(foo,"%d--%d [style=dotted,color=green];\n",dest,v[dest].par);
        if(a[dest][v[dest].par]==3)
        	fprintf(foo,"%d--%d [style=dotted, color=blue];\n",dest,v[dest].par);
...
void am_to_dot_black(int a[][100],int size,FILE *foo){
    ...
    fo(i,0,size){
        ...
        if(i<=j && a[i][j]!=0){
        	...
        	if(a[i][j]==2)
        		fprintf(foo,"%d--%d [color=green];\n",i,j);
                if(a[i][j]==3)
       			fprintf(foo,"%d--%d [color=blue];\n",i,j);
                count++;
            }
        }
        if(count ==0){
